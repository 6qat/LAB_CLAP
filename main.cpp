...
static bool my_plug_init(const struct clap_plugin *plugin) {
	...
	plug->hostThreadCheck =
		(const clap_host_thread_check_t *)plug->host->get_extension(plug->host, CLAP_EXT_THREAD_CHECK);
	plug->hostLatency = (const clap_host_latency_t *)plug->host->get_extension(plug->host, CLAP_EXT_LATENCY);
	return true;
}
...
static void my_plug_process_event(my_plug_t *plug, const clap_event_header_t *hdr) {
	if (hdr->space_id == CLAP_CORE_EVENT_SPACE_ID) {
		...
		case CLAP_EVENT_NOTE_ON: {
			const auto *ev = (const clap_event_note_t *)hdr;
			// TODO: handle note on
			break;
		}
		...
		case CLAP_EVENT_NOTE_EXPRESSION: {
			const auto *ev = (const clap_event_note_expression_t *)hdr;
			// TODO: handle note expression
			break;
		}

		case CLAP_EVENT_PARAM_VALUE: {
			const auto *ev = (const clap_event_param_value_t *)hdr;
			// TODO: handle parameter change
			break;
		}

		case CLAP_EVENT_PARAM_MOD: {
			const auto *ev = (const clap_event_param_mod_t *)hdr;
			// TODO: handle parameter modulation
			break;
		}

		case CLAP_EVENT_TRANSPORT: {
			const auto *ev = (const clap_event_transport_t *)hdr;
			// TODO: handle transport event
			break;
		}

		case CLAP_EVENT_MIDI: {
			const auto *ev = (const clap_event_midi_t *)hdr;
			// TODO: handle MIDI event
			break;
		}

		case CLAP_EVENT_MIDI_SYSEX: {
			const auto *ev = (const clap_event_midi_sysex_t *)hdr;
			// TODO: handle MIDI Sysex event
			break;
		}

		case CLAP_EVENT_MIDI2: {
			const auto *ev = (const clap_event_midi2_t *)hdr;
